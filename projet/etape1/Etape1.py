...
class Etape1:
    ...
    # //////////////////////////////////////////////
    def Clausephi(self, x, sommet):
        clauseCourant = []
        couleurTraitee = []
        # clause ( Ji ou Ri ou Bi ou ... CNi)
        for couleur in range(1, x + 1):
            clauseCourant.append(x * sommet + couleur)
        self.base.append(clauseCourant)
        # clause couple de couleur
        for couleur in range(1, x + 1):
            couleurTraitee.append(couleur)
            for couleurDeux in range(1, x + 1):
                if (couleurDeux not in couleurTraitee):
                    clause = [-(x * sommet + couleur), -(x * sommet + couleurDeux)]
                    self.base.append(clause)

    def Clausepsi(self,x,sommet,sommetAdjacent):
        for couleur in range(1, x + 1):
            clause = [-(x * sommet + couleur), -(x * sommetAdjacent + couleur)]
            self.base.append(clause)
    ...
    def majBase(self, x: int):
        ...
        :param x: parametre servant pour definir la base qui representera le probleme,nombre de couleur
        """
        # A ECRIRE par les etudiants en utilisant le contenu de g
        # ajout possible de parametre => modifier aussi l'appel ds le main
        # on a une variable de par sommet associées a chaque couleurs
        self.base=[]
        self.nbVariables = x * self.g.getNbSommets()
        SommetVisitee = []
        for sommet in self.g.getSommets():
            #clause phi
            self.Clausephi(x, sommet)
            #clause psi
            for sommetAdjacent in self.g.getAdjacents(sommet):
                if(sommetAdjacent in SommetVisitee ):
                   self.Clausepsi(x,sommet,sommetAdjacent)
            SommetVisitee.append(sommet)
